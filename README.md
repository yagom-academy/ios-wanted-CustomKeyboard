...
|기존 상태 목록|새로운 상태 목록|
|---|---|
|`시작`<br>`초성`<br>`중성`<br>`겹중성`<br>`종성`<br>`겹종성`<br>`다음 조합 버퍼로 문자 하나를 넘기는 상태(이하 finishPassOne)`<br>`다음 조합 버퍼로 문자 두 개를 넘기는 상태`| - 조합 상태: <br>`진행중`<br>`끝남` <br> <br>- 각 문자의 위치: <br>`초성`<br>`중성`<br>`중성`|
- 기존 상태 목록에서는 현재 조합 상태와 각 문자의 위치에 관한 정보가 섞여 있었다. 
- 그렇기 때문에 만약 현재 문자의 상태가 한번 정해졌다해도, 경우에 따라 문자 조합 과정이 끝난 후 문자의 상태를 다시 정해줘야 하는 일이 생겼다.(ex. finishPassOne이 상태로 한번 정해진 경우) 
- 하지만 새로운 상태 목록에서는 조합 상태와 각 문자의 위치를 각각 따로 저장한다. 
- 따라서 입력 처리 중에 한번만 문자의 상태를 정해주면 된다.

#### 2차 리팩토링 후기
- 리팩토링 이후, 한 메서드의 수정이 다른 메서드에 큰 영향을 미치지 않게 되었다.
- 클래스별로 확실한 단일 책임을 부여했기 때문에 에러가 나더라도 해당 에러의 원인이 어느 클래스인지를 바로 파악할 수 있어 작업하기 수월했다.
#### 3, 4차 리팩토링
- 코드 중복을 없애고 의미 전달을 명확하게 하는 걸 목표로 리팩토링 진행
- `API Design Guidelines`를 참고하여 변수/메서드/클래스명과 메서드 파라미터 수정
- guard let과, if문, switch문의 적절한 사용에 대해 깊이 생각해볼 수 있어서 유익했다.
