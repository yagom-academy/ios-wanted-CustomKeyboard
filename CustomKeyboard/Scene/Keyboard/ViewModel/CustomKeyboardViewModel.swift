...
final class CustomKeyboardViewModel {
  ...
  func addChar(_ InputCode: Int ){
    guard let inputChar = UnicodeScalar(InputCode)?.description
    else {
      text.value = ""
      return
    }
    guard let lastChar = text.value.last,
          let lastCharUnicode = UnicodeScalar(String(lastChar))?.value
    else {
