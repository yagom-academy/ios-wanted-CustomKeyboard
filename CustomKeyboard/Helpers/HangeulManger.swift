...
final class HangeulManger: UnicodeManger {
  ...
  private func combineIncludingFinalChar(lastChar: HangeulStruct, inputhChar: Int) -> String {
    if inputhChar <= CharUnicode.ㅎ.value {
      ...
      if combineCharValue == 0 {
        let lastChar = converter.combineCharToUnicode(initial: lastChar.initial,
                                                      neutral: lastChar.neutral,
                                                      final: final)
  ...
  private func combineFinalToConsonant(lastChar: HangeulStruct, consonant: Int) -> Int {
    ...
    case Final.ㄱ.value :
      if initialValue == Initial.ㄱ.value {
        lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                         neutral: lastChar.neutral,
                                                         final: Final.ㄲ.value)
  ...
  private func combineFinalToVowel(lastChar: HangeulStruct,vowel: Int) -> String {
    ...
    case Final.ㄲ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄱ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㄱ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄳ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄱ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅅ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄵ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄴ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅈ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄶ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄴ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅎ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄺ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
    ...
    case Final.ㄻ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄹ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅁ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄼ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄹ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅂ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㄽ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
    ...
    case Final.ㄾ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: Final.ㄹ.value)
      nextCharUnicode = converter.combineCharToUnicode(initial: Initial.ㅌ.value,
                                                       neutral: neutral,
                                                       final: 0)
    case Final.ㅀ.value:
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
    ...
    default:
      ...
      lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                       neutral: lastChar.neutral,
                                                       final: 0)
      nextCharUnicode = converter.combineCharToUnicode(initial: paringValue,
                                                       neutral: neutral,
                                                       final: 0)
    }
  ...
  private func combineNoneFinalChar(lastChar: HangeulStruct, inputChar: Int) -> String {
    if inputChar <= CharUnicode.ㅎ.value {
      ...
      if combineCharUnicode == 0 {
        let lastCharUnicode = converter.combineCharToUnicode(initial: lastChar.initial,
                                                             neutral: lastChar.neutral,
                                                             final: 0)
  ...
  private func combineConsonantToVowel(consonant: Int, inputChar: Int) -> String {
    let neutralValue = Neutral.getVowelToNeutralValue(inputChar)
    let initialValue = Initial.getConsonantToInitialValue(consonant)
    let lastCharUnicode = converter.combineCharToUnicode(initial: initialValue,
